...
const DEPLOY_NOTIFICATION_STATUS_URL = "deployment-status.json";
...
function appendDeployAlertControl(target) {
  ...
  enable.addEventListener("click", async () => {
    ...
    if (permission === "granted") {
      try {
        localStorage.setItem(DEPLOY_NOTIFICATION_SEEN_KEY, BUILD_SHA);
      } catch (_) {}
...
function seedDeploymentNotificationState(storage = typeof localStorage !== "undefined" ? localStorage : null) {
  if (!storage || !BUILD_SHA || BUILD_SHA.startsWith("__")) return;
  try {
    if (!storage.getItem(DEPLOY_NOTIFICATION_SEEN_KEY)) storage.setItem(DEPLOY_NOTIFICATION_SEEN_KEY, BUILD_SHA);
  } catch (_) {}
}
...
async function checkDeploymentNotification({
  ...
  deploymentNotificationPromise = (async () => {
    const nonce = Number(typeof now === "function" ? now() : Date.now());
    const response = await fetchImpl(`./${DEPLOY_NOTIFICATION_STATUS_URL}?watch=${nonce}`, { cache: "no-cache" });
    if (!response.ok) return null;
    const payload = await response.json();
    ...
    try {
      seenSha = storage?.getItem(DEPLOY_NOTIFICATION_SEEN_KEY) || BUILD_SHA;
    } catch (_) {}
    if (liveSha === seenSha) return payload;

    try {
      storage?.setItem(DEPLOY_NOTIFICATION_SEEN_KEY, liveSha);
    } catch (_) {}

    if (liveSha !== BUILD_SHA) showUpdateReady(locationObj);
    if (payload?.notify_when_deployed === true) {
    ...
    .catch(() => null)
    .finally(() => {
      deploymentNotificationPromise = null;
    });
...
async function boot() {
  seedDeploymentNotificationState();
  updateSiteMeta();
  void refreshProductionStatus();
  void checkDeploymentNotification();
