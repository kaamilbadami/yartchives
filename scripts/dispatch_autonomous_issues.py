...
def session_prompt(
    ...
    prompt = (
        ...
        "coverage where behavior changes. When intended behavior changes an existing contract, " 
        "update or replace the old regression assertions in the same change; do not leave " 
        "stale tests that encode superseded behavior. Before opening the pull request, run the smallest "
        "relevant regression tests, push the branch, and require the exact branch head SHA "
        "to pass the repository's targeted 'Branch preflight' checks. If preflight "
        "fails, repair the same branch and rerun it; do not publish a knowingly failing PR. "
        "Do not ask for confirmation merely to continue "
