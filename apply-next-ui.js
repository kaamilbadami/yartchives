(function (root, factory) {
  ...
  function timingDiagnosticText(payload) {
    ...
    for (const [name, value] of Object.entries(payload.stages_ms || {})) {
      const label = TIMING_STAGE_LABELS[name] || name;
      lines.push(`${label}: ${Number(value || 0).toFixed(1)} ms`);
    }
  ...
  function renderTimingDiagnostic(panel, payload) {
    ...
    const output = element("pre", "apply-next-diagnostics-output", timingDiagnosticText(payload));
    ...
    details.append(note, output, copy);
