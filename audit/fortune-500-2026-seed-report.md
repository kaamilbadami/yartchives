...
| Merged into existing employer identities | 10 |
| Newly added employers | 490 |
| Resulting employer-universe size | 546 |

The ten merged identities are American Express, Amgen, BNY, CACI, DoorDash, Exelon, Interactive Brokers, Leidos, Stanley Black & Decker, and The Hartford. Existing display names and benchmark metadata remain canonical; Fortune names and ranks are added as aliases and seed-specific metadata where applicable.
...
The three immediately ready identities are CACI, Leidos, and The Hartford. DoorDash retains a shared Greenhouse-domain hint and is `needs_tenant_identity`. These hints come only from previously preserved benchmark evidence; the Fortune catalog contributes none.

Across the full 546-employer unresolved universe, the corresponding counts are 20 `ready`, 13 `needs_tenant_identity`, and 513 `no_domain_hint`.

## Identity findings

The deterministic alias rules produced 92 aliases across 88 catalog records and caused five of the ten existing-identity merges. No identity collisions were found. Parenthetical names supply aliases such as `BNY`; terminal corporate descriptors supply aliases such as `Leidos` from `Leidos Holdings`; and a leading `The` is ignored for matching.
