...
(async () => {
  ...
  assert.ok(uiSource.indexOf("if (!feedReadyForRecommendations())") < uiSource.indexOf("const pool = candidatePool(feed.jobs"), "Feed readiness must be checked before candidate filtering");
