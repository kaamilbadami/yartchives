...
const noDateJob = { title: "Software Engineer Intern", profiles: ["cs"], term: "Summer 2027", company: "Z-NoDate", posted_at: null, link_kind: "direct", states: ["CT"] };
...
assert.equal(recommendedRanked.length, 5); // graduateOnly is excluded
assert.equal(recommendedRanked[0].job.company, "A"); // Highest total score (direct, CT, dated)
assert.equal(recommendedRanked[1].job.company, "Y-InvalidDate"); // High score (direct/CT), but no date
assert.equal(recommendedRanked[2].job.company, "Z-NoDate"); // High score, invalid date
assert.equal(recommendedRanked[3].job.company, "B"); // Lower score tie
assert.equal(recommendedRanked[4].job.company, "B-clone"); // Lower score tie
...
assert.equal(newestRanked[0].job.company, "A"); // Newest date
assert.equal(newestRanked[1].job.company, "B"); // Older date tie-break by title/company
assert.equal(newestRanked[2].job.company, "B-clone");
assert.equal(newestRanked[3].job.company, "Y-InvalidDate"); // Fallback date 0, tie-break by total/company
